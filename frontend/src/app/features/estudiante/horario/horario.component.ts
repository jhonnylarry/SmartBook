...
/** Pestaña "Mi Horario" del estudiante: su horario semanal de clases. */
@Component({
  ...
  ngOnInit(): void {
    this.estudianteApi.me().pipe(
      switchMap((detalle) => {
        ...
        if (!matricula) return of([] as BloqueHorarioDto[]);
        return this.horarioApi.porCurso(matricula.idCurso).pipe(catchError(() => of([] as BloqueHorarioDto[])));
      }),
