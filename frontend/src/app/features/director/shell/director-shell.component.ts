...
@Component({
  ...
  template: `
    <div class="flex flex-col h-screen overflow-hidden" style="background: var(--color-surface);">
      ...
      >
        <div class="max-w-[1400px] mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-3">
          ...
          <!-- ── Marca ── -->
          <a routerLink="/director/dashboard" class="flex items-center gap-2.5 shrink-0 group" aria-label="Ir al inicio">
      ...
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true" aria-label="Secciones">
        ...
        >
          ...
          <div class="border-t border-slate-100 px-6 py-3 flex items-center gap-4 text-xs text-slate-400">
            ...
            <span class="flex items-center gap-1.5"><kbd class="font-mono border border-slate-200 rounded px-1.5 py-0.5 bg-white">⌘K</kbd> buscar acciones</span>
          </div>
        </div>
      </div>
    }
    ...
    <!-- Command Palette (búsqueda / acciones rápidas: FAB + ⌘K) -->
    <app-command-palette />
  `,
  styles: [`
...
export class DirectorShellComponent implements OnDestroy {
  ...
  readonly navItems: NavItem[] = [
    {
      ...
      description: 'Resumen general del colegio',
      ...
      gradFrom: '#2547A0', gradTo: '#13294B',
    },
    {
      label: 'Matrícula',
      route: '/director/matricula',
    ...
    {
      label: 'Anotaciones',
      route: '/director/anotaciones',
      description: 'Registro conductual del estudiante',
    ...
    {
      label: 'Periodos',
      route: '/director/periodos',
      description: 'Trimestres/semestres y cierre académico',
      svgPath: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z',
    ...
    {
      label: 'Mensajes',
      route: '/director/mensajes',
      description: 'Comunicación interna del colegio',
      svgPath: 'M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z',
      gradFrom: '#0891b2', gradTo: '#0e7490',
    },
  ];

  visibleNavItems(): NavItem[] {
    return this.navItems.filter((i) => i.cap !== 'usuarios' || this.auth.canViewUsuarios());
  }
