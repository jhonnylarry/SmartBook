...
/** Shell del workspace del Apoderado/Tutor. La barra superior es la navegación única (Mis Pupilos · Notificaciones). */
@Component({
  selector: 'app-apoderado-shell',
  standalone: true,
    ...
    <div class="flex flex-col h-screen overflow-hidden" style="background: var(--color-surface);">
      ...
      >
        <div class="max-w-[1400px] mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-3">
          ...
          <a routerLink="/apoderado/inicio" class="flex items-center gap-2.5 shrink-0 group" aria-label="Inicio">
          ...
          <div class="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <a routerLink="/apoderado/pupilos"
            ...
            <a routerLink="/apoderado/notificaciones"
              routerLinkActive="!text-primary-700 !bg-primary-50 !border-primary-100"
              class="hidden sm:flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium text-slate-600
                     hover:text-primary-700 hover:bg-primary-50 border border-transparent hover:border-primary-100 transition-all duration-150">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75" aria-hidden="true">
            ...
            <a routerLink="/apoderado/mensajes"
              routerLinkActive="!text-primary-700 !bg-primary-50 !border-primary-100"
              class="hidden sm:flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium text-slate-600
                     hover:text-primary-700 hover:bg-primary-50 border border-transparent hover:border-primary-100 transition-all duration-150">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75" aria-hidden="true">
            ...
            <app-campana-notificaciones [inicioLink]="'/apoderado/notificaciones'" />
              ...
              <div class="relative">
                ...
                >
                  <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0"
                    style="background: linear-gradient(135deg, #1E3A8A 0%, #2547A0 100%); box-shadow: 0 2px 8px rgba(19,41,75,0.3);" aria-hidden="true">
                  ...
                  <svg class="hidden sm:block w-3.5 h-3.5 text-slate-400 transition-transform duration-150 shrink-0"
                    [class.rotate-180]="userMenuOpen()" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                  </svg>
                </button>

                @if (userMenuOpen()) {
                  <div class="absolute right-0 top-full mt-2 w-56 rounded-2xl py-2 z-30 animate-slideDown"
                    ...
                    <div class="px-4 py-3 border-b border-slate-100">
                      ...
                      <div class="mt-2"><app-badge-rol [rol]="user.rol" /></div>
                    </div>
                    <div class="px-2 pt-2">
                      <button (click)="logout()"
                        class="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm text-red-600 hover:bg-red-50 hover:text-red-700 transition-colors duration-150"
                        role="menuitem">
...
export class ApoderadoShellComponent {
